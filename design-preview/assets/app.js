...
(function () {
  ...
  function init() {
    ...
    const hdr = document.getElementById("hdr"), totop = document.getElementById("totop");
    ...
    const cio = new IntersectionObserver((es) => es.forEach(e => {
      ...
      const el = e.target, end = +el.dataset.count, suf = el.dataset.suffix || ""; let t0 = null; const dur = 1500;
      const step = (t) => { if (!t0) t0 = t; const p = Math.min((t - t0) / dur, 1); const v = Math.floor((1 - Math.pow(1 - p, 3)) * end); el.textContent = v.toLocaleString("en-IN") + suf; if (p < 1) requestAnimationFrame(step); };
    ...
    document.querySelectorAll(".reveal-words").forEach(el => {
      ...
      walk(el);
      el.querySelectorAll(".word>span").forEach((sp, i) => sp.style.transitionDelay = (i * 0.05) + "s");
    });
    ...
    document.querySelectorAll(".tilt").forEach(el => {
      el.addEventListener("mousemove", e => {
        ...
        const ry = (px - 0.5) * 15, rx = (0.5 - py) * 15;
        el.style.transform = "perspective(1200px) rotateX(" + rx + "deg) rotateY(" + ry + "deg) translateZ(34px) scale(1.045)";
      });
    ...
    document.querySelectorAll(".thumbs .frame").forEach(t => t.addEventListener("click", () => {
      document.querySelector(".thumbs .frame.active")?.classList.remove("active");
      t.classList.add("active");
      const m = document.querySelector(".main-img"); if (m) m.setAttribute("data-label", (t.getAttribute("data-label") || "") + " — enlarged");
    }));
    document.querySelectorAll(".qty button").forEach(b => b.addEventListener("click", () => {
      const s = b.parentElement.querySelector("span"); let n = +s.textContent;
      n = b.textContent.trim() === "+" ? n + 1 : Math.max(1, n - 1); s.textContent = n;
    }));
    // filter chips
    document.querySelectorAll(".chip").forEach(c => c.addEventListener("click", () => {
      document.querySelector(".chip.active")?.classList.remove("active"); c.classList.add("active");
    }));
  }
