...
export const Route = createFileRoute("/services")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Commission handmade pencil sketches, colour portraits, paintings, mirror art, clay art and personalized gifts from Artspire India. Every artwork made by hand. Starting from ₹799. WhatsApp to order." },
    ],
  }),
...
function ServiceCard({ s }: { s: Service }) {
  ...
  return (
    <article className="bg-white rounded-xl overflow-hidden border border-border/60 hover-lift shadow-sm flex flex-col h-full">
      <div className="h-[180px] md:h-[200px] overflow-hidden bg-surface-variant">
        <img src={s.image} alt={s.headline} loading="lazy" className="w-full h-full object-cover img-zoom" />
      </div>
      <div className="p-5 md:p-6 flex flex-col flex-grow">
        <p className="font-body text-[10px] font-bold uppercase tracking-[0.2em] text-gold mb-2">{s.label}</p>
        <h3 className="font-display text-[18px] md:text-[20px] text-forest font-bold leading-tight mb-2">{s.headline}</h3>
        <p className="font-accent italic text-[13px] text-stone leading-snug mb-3">{s.tagline}</p>
        <div className="flex flex-wrap gap-1.5 mb-3">
          {s.tags.map((t) => (
            <span key={t} className="rounded-full border border-stone/30 text-stone text-[10px] font-body px-2.5 py-0.5">{t}</span>
          ))}
        </div>
        <p className={`text-[13px] text-charcoal leading-relaxed mb-3 ${expanded ? "" : "line-clamp-2"}`}>
          {s.description}
        </p>
        <button
          type="button"
          onClick={() => setExpanded((v) => !v)}
          className="text-left text-gold text-[12px] font-semibold mb-4 hover:text-gold-dark transition-colors"
        >
          {expanded ? "Read less ↑" : "Read more →"}
        </button>
        <div className="flex items-center justify-between pt-3 border-t border-border/40 mt-auto">
          <span className="text-gold font-bold text-[14px]">{s.price}</span>
          <span className="text-stone text-[12px]">⏱ {s.timeline}</span>
        </div>
        <a
          href={waLink(s.message)}
          target="_blank"
          rel="noreferrer"
          className="flex items-center justify-center gap-2 w-full h-[44px] mt-4 rounded-lg bg-forest text-white text-[12px] font-bold uppercase tracking-wide hover:bg-forest-dark transition-colors active-scale btn-primary"
        >
      ...
      <div className="bg-cream-dark">
        ...
        <section className="section-padding text-center">
          <div className="container-main">
            <p className="font-body text-[11px] md:text-[12px] font-semibold text-gold mb-4 uppercase tracking-[0.25em]">What I Create</p>
            <h1 className="font-display text-[32px] md:text-[42px] font-bold text-forest mb-4 leading-tight">Handcrafted Art for Every Occasion</h1>
            <p className="font-body text-[14px] md:text-[16px] text-stone max-w-xl mx-auto">Every piece is made by hand. Every commission begins with your story.</p>
          </div>
        </section>
        ...
        <section className="pb-12 md:pb-16">
          <div className="container-main">
            <div className="bg-cream rounded-xl border-l-4 border-gold p-6 md:p-8">
              <h2 className="font-display text-[24px] md:text-[28px] text-forest font-bold mb-2">Our Promise to You</h2>
              <p className="font-body text-[14px] text-stone mb-6">Every order comes with complete clarity on timing, revisions and delivery.</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
                {promises.map((p) => (
                  <div key={p.title} className={`bg-white rounded-xl p-4 md:p-5 border ${p.highlight ? "border-gold/40" : "border-border/40"}`}>
                    <div className="text-[20px] mb-2">{p.icon}</div>
                    <p className="font-display text-[15px] md:text-[16px] text-forest font-semibold mb-1">{p.title}</p>
                    <p className="font-body text-[12px] md:text-[13px] text-stone leading-relaxed">{p.body}</p>
                  </div>
                ))}
              </div>
              <div className="text-center mt-6 md:mt-8">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 h-[48px] px-8 bg-forest text-white text-[13px] font-bold uppercase tracking-wide rounded-xl hover:bg-forest-dark transition-colors active-scale btn-primary"
                >
                  Ask on WhatsApp
                </a>
              </div>
        ...
        <section className="section-padding bg-forest text-center">
          <div className="container-main max-w-2xl">
            <h2 className="font-display text-[26px] md:text-[32px] text-white font-bold mb-4 leading-tight">Not Sure Which Art Is Right for Your Occasion?</h2>
            <p className="font-body text-[14px] md:text-[16px] text-cream/70 mb-8 leading-relaxed">Tell me who it is for, what the occasion is, and your budget. I will tell you exactly what will make them remember this gift for the rest of their life.</p>
            <a
              href={waLink("Hi Artspire! I'm not sure which artwork type would work best for my occasion. Can you help me decide?")}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 h-[52px] px-8 bg-gold text-forest text-[13px] md:text-[14px] font-bold uppercase tracking-wide rounded-full btn-gold transition-colors active-scale"
            >
