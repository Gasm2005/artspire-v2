...
import { Search, Globe, FileText, Image, CheckCircle, AlertCircle, XCircle, ChevronDown, ChevronUp, Loader2, Save, Eye } from "lucide-react";
import { getWebsiteContent, upsertWebsiteContent, updateWebsiteContent, type WebsiteContent } from "@/lib/website-content";
...
function GooglePreview({ title, description, url }: { title: string; description: string; url: string }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-border p-4 font-body">
      <p className="text-[11px] text-stone/50 mb-2 uppercase tracking-wider font-semibold">Google Preview</p>
      <div className="text-[12px] text-stone/60 mb-0.5">{siteUrl}{url}</div>
...
function SEOFieldEditor({
  titleKey, descKey, ogImageKey,
  titleDefault, descDefault,
  url, saving, onSave,
  initialTitle, initialDesc, initialOg,
}: {
  titleKey: string; descKey: string; ogImageKey?: string;
  titleDefault: string; descDefault: string;
  url: string; saving: boolean;
  onSave: (key: string, val: string) => Promise<void>;
  initialTitle: string; initialDesc: string; initialOg?: string;
}) {
  ...
  useEffect(() => { setOg(initialOg ?? ""); }, [initialOg]);
  ...
  const inputClass = "w-full px-4 py-2.5 rounded-xl border border-border bg-white font-body text-[13px] text-forest focus:outline-none focus:border-gold transition-colors";
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`font-body text-[11px] font-semibold ${charColor(title.length, 30, 65)}`}>
            {title.length}/65
          </span>
        </div>
        <input
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`font-body text-[11px] font-semibold ${charColor(desc.length, 80, 165)}`}>
            {desc.length}/165
          </span>
        </div>
        <textarea
      ...
      {ogImageKey && (
        <div>
          ...
          <input
            ...
            className={inputClass}
          />
          <p className="font-body text-[10px] text-stone/50 mt-1">Shown when shared on WhatsApp, Instagram, Facebook</p>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-border">
        ...
        {saved && <span className="font-body text-[12px] text-green-600 font-semibold">✓ Saved!</span>}
      </div>
    </div>
  );
}
...
// ─── Accordion wrapper ───────────────────────────────────────
function Accordion({ title, subtitle, score, children }: { title: string; subtitle: string; score: "good" | "warn" | "missing"; children: React.ReactNode }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
      ...
      >
        ...
        {open ? <ChevronUp size={16} className="text-stone/40" /> : <ChevronDown size={16} className="text-stone/40" />}
      </button>
...
function SEOPage() {
  ...
  const saveContent = useCallback(async (key: string, val: string) => {
  ...
  const saveArtworkSEO = useCallback(async (id: string, field: string, val: string) => {
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      {/* Header */}
      <div>
        <h1 className="font-display text-[24px] md:text-[28px] text-forest font-medium">SEO Center</h1>
        <p className="font-body text-[13px] text-stone mt-0.5">Meta tags, descriptions, and structured data for every page</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="bg-white rounded-xl border border-border p-4 text-center shadow-sm">
          <p className="font-display text-[28px] text-amber-500 font-medium">{pageScores.filter(s => s === "warn").length}</p>
      ...
      {tab === "artworks" && (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <span className="font-body text-[12px] text-stone/50 shrink-0">{filteredArtworks.length} artworks</span>
          </div>
      ...
      {tab === "global" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-border p-5 shadow-sm space-y-4">
            <GlobalField label="Default Site Title" hint="Used when no page title is set" contentKey="seo.global.site_title" placeholder="Artspire | Custom Handmade Art by Himangi Pandey" cv={cv} save={saveContent} />
            <GlobalField label="Default Meta Description" hint="Fallback for all pages" contentKey="seo.global.site_description" placeholder="Custom handmade pencil sketches, paintings, clay art, and mirror art from your photos. Delivered across India." cv={cv} save={saveContent} textarea />
            <GlobalField label="Default OG Image URL" hint="Shown when any page is shared on social media (1200×630px)" contentKey="seo.global.og_image" placeholder="https://artspire.in/og-default.jpg" cv={cv} save={saveContent} />
            <GlobalField label="Twitter / X Handle" hint="e.g. @artspirein" contentKey="seo.global.twitter_handle" placeholder="@artspirein" cv={cv} save={saveContent} />
            <GlobalField label="Google Search Console Verification" hint="Paste the content= value from Google Search Console meta tag" contentKey="seo.global.google_verification" placeholder="xxxxxxxxxxxxxxxxxxxxxx" cv={cv} save={saveContent} />
          </div>

          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <p className="font-body text-[12px] font-semibold text-amber-800 mb-1">📍 Already done automatically</p>
          ...
          <div className="bg-white rounded-2xl border border-border p-5 shadow-sm">
            <p className="font-body text-[13px] font-semibold text-forest mb-3">🔗 Submit to search engines</p>
            <div className="space-y-2">
              <a href="https://search.google.com/search-console" target="_blank" rel="noreferrer" className="flex items-center justify-between p-3 rounded-xl border border-border hover:border-forest/30 transition-colors group">
                <div>
                  <p className="font-body text-[13px] font-semibold text-forest">Google Search Console</p>
                  <p className="font-body text-[11px] text-stone/60">Submit sitemap, monitor rankings</p>
                </div>
                <span className="font-body text-[11px] text-gold font-semibold group-hover:text-forest">Open →</span>
              </a>
              <a href="https://www.bing.com/webmasters" target="_blank" rel="noreferrer" className="flex items-center justify-between p-3 rounded-xl border border-border hover:border-forest/30 transition-colors group">
                <div>
                  <p className="font-body text-[13px] font-semibold text-forest">Bing Webmaster Tools</p>
                  <p className="font-body text-[11px] text-stone/60">Submit sitemap to Bing & DuckDuckGo</p>
                </div>
                <span className="font-body text-[11px] text-gold font-semibold group-hover:text-forest">Open →</span>
              </a>
            </div>
            <p className="font-body text-[11px] text-stone/50 mt-3">
              Sitemap URL to submit: <span className="font-semibold text-forest">https://artspire.in/sitemap.xml</span>
            </p>
          </div>
        </div>
      )}
...
function ArtworkSEOEditor({ artwork, initialTitle, initialDesc, onSave }: {
  ...
  return (
    <div className="space-y-4">
      {artwork.image_url && (
        <img src={artwork.image_url} alt={artwork.title} className="w-full h-[120px] object-cover rounded-xl" />
      )}
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label className="font-body text-[11px] font-bold text-stone uppercase tracking-wider flex items-center gap-1.5">
          ...
          <span className={`font-body text-[11px] font-semibold ${charColor(title.length, 30, 65)}`}>{title.length}/65</span>
        </div>
        <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={`${artwork.title} | Artspire`} maxLength={80} className={inputClass} />
        <p className="font-body text-[10px] text-stone/50 mt-1">If empty, uses artwork title automatically</p>
      </div>
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label className="font-body text-[11px] font-bold text-stone uppercase tracking-wider flex items-center gap-1.5">
          ...
          <span className={`font-body text-[11px] font-semibold ${charColor(desc.length, 80, 165)}`}>{desc.length}/165</span>
        </div>
        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={artwork.summary ?? `Custom handmade ${artwork.title} by Artspire artist Himangi Pandey.`} maxLength={200} rows={3} className={inputClass + " resize-none"} />
      </div>
      <button onClick={() => setShowPreview(!showPreview)} className="flex items-center gap-1.5 font-body text-[11px] text-forest font-semibold hover:text-gold transition-colors">
        <Eye size={13} />
        {showPreview ? "Hide" : "Show"} Google Preview
      </button>
      {showPreview && <GooglePreview title={title || `${artwork.title} | Artspire`} description={desc || artwork.summary || ""} url={`/artwork/${artwork.slug}`} />}
      <div className="flex items-center gap-3 pt-2 border-t border-border">
        <button onClick={handleSave} disabled={saving} className="inline-flex items-center gap-2 h-[40px] px-5 bg-forest text-white font-body font-bold text-[12px] rounded-xl hover:bg-forest/90 disabled:opacity-50 transition-colors">
...
function GlobalField({ label, hint, contentKey, placeholder, cv, save, textarea }: {
  label: string; hint: string; contentKey: string; placeholder: string;
  cv: (k: string) => string; save: (k: string, v: string) => Promise<void>;
  textarea?: boolean;
}) {
  ...
  useEffect(() => { setVal(cv(contentKey)); }, [cv, contentKey]);
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-start justify-between">
        <div>
          <label className="font-body text-[11px] font-bold text-stone uppercase tracking-wider">{label}</label>
          <p className="font-body text-[10px] text-stone/50">{hint}</p>
        </div>
        {saved && <span className="font-body text-[11px] text-green-600 font-semibold shrink-0">✓ Saved</span>}
      </div>
      {textarea ? (
        <textarea value={val} onChange={(e) => setVal(e.target.value)} placeholder={placeholder} rows={3} className={inputClass + " resize-none"} />
      ) : (
        <input type="text" value={val} onChange={(e) => setVal(e.target.value)} placeholder={placeholder} className={inputClass} />
      )}
      <button onClick={handleSave} disabled={saving} className="inline-flex items-center gap-1.5 h-[36px] px-4 bg-forest text-white font-body font-semibold text-[11px] rounded-lg hover:bg-forest/90 disabled:opacity-50 transition-colors">
